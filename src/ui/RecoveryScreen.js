...
import RecoveryPresenter from "../presenter/RecoveryPresenter";
...
export default class RecoveryScreen extends BaseApp {
	...
	getPresenter(): BasePresenter {
		mPresenter = RecoveryPresenter.getInstance(this);
		return mPresenter;
	}

	onLoad() {
        super.onLoad();
        mPresenter.detail({orderId: this.state.orderId})
	}
    ...
    recovery() {
        mPresenter.recovery({orderId: this.state.order.orderId, evidence: this.state.evidenceUrl});
    }
            ...
            <View style={{flex: 1, backgroundColor: "#F3F3F3"}}>
                ...
                <View style={{alignItems: 'center'}}>
                    ...
                    <MyButtonView style={{width: width / 1.3, marginTop: px2dp(75.5)}} modal={1}
                              title={'操作完成'}
                              onPress={this.recovery.bind(this)}/>
                </View>
                </ScrollView>
            </View>
        )
