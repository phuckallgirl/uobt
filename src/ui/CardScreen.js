...
import CardPresenter from "../presenter/CardPresenter"
...
class ListItem extends Component {
    ...
    render() {
        let {onPress, bank, account, number} = this.props
        onPress = onPress || (() => {})
        return (
            <View style={Styles.item_container}>
                <TouchableOpacity onPress={this.props.onPress}>
                    <View style={Styles.topicView}>
                        <Text style={Styles.topic}>{account} - {bank}</Text>
                    </View>
...
export default class CardScreen extends BaseApp{
	...
	getPresenter(): BasePresenter {
		mPresenter = CardPresenter.getInstance(this);
		return mPresenter;
	}
	...
	renderItem = (data) => {
		return(
			<ListItem bank={data.item.bank} account={data.item.account} number={data.item.card_no}/>
		);
	}
	...
	render(){
		return(
			<View style={Styles.container}>
				<View>
					<TabHeader bgcolor="#1E82D2" text="银行卡" code="add" rightPress={this.goAddCard.bind(this)} back right />
				</View>
				<SpacingView/>
